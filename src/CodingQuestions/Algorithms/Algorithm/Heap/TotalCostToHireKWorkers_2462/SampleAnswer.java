package CodingQuestions.Algorithms.Algorithm.Heap.TotalCostToHireKWorkers_2462;
...
public class SampleAnswer {
    ...
    public long totalCost(int[] costs, int k, int candidates) {
        ...
        for (int i = 0; i < candidates; i++) headWorkers.add(costs[i]);
        for (int i = Math.max(candidates, costs.length - candidates); i < costs.length; i++) tailWorkers.add(costs[i]);
